...
## 0. Mandatory Code-Change Agent

- 本工作区任何涉及代码创建、修改、删除、重构、优化、补测试的请求，必须优先通过自定义 agent `Workspace Implementation Guardian` 执行。
- 该 agent 定义位于 `.github/agents/workspace-implementation-guardian.agent.md`，它是本工作区代码改动流程的唯一默认执行入口。
- 如果当前任务只是讨论方案、解释代码、评审风险且不落代码，可以保留在默认对话；一旦进入实际代码改动，必须切换或委派给该 agent。
- 该 agent 中定义的 7 项编程原则、命名规范、数据校验、few-shot、单元测试与复用优化要求，优先作为本仓库代码改动的执行准则。

### 0.1 Task Slicing And Change Scope

- 任何开发任务默认必须先按可执行子任务切片推进，而不是把一个跨层需求一次性做完。
- 以下“细化任务拆分”默认以非测试代码为口径控制粒度；测试补齐跟随对应实现切片一起完成，但不单独作为拆分依据。
- 单个子任务的目标范围是 100 到 200 行左右的非测试代码改动；如果预估超过 300 行，则必须继续向下拆分。
- 单个子任务尽量只影响 1 个 owner 文件加少量直接依赖文件，避免一次横跨前端、BFF、Mastra 三层。
- 需要跨层联动的功能，优先拆成“契约定义”“单层接入”“状态透传”“展示或落盘”四类子任务，分别落地。
- 如果用户让你“继续完成当前任务”，默认含义是继续完成当前最小未完成子任务，而不是自动把同一大任务下所有跨层子项并到一个提交切片里。
- 除非用户明确要求整包交付，或者当前阻塞只能通过跨层同时改动解除，否则禁止把多个子任务合并成一次跨层改动。
- 已完成任务保持原有完成状态不变；如果为了继续演进而新增更细的拆分，优先把新增拆分和当前完成情况同步回计划或 instruction，而不是覆盖原状态。
