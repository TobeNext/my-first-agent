...
- `interview-question-generator.ts` 负责两类生成行为：一类是把召回结果适配成初始化主问题集合与 generation trace，另一类是基于当前题、bounded follow-up memory context（历史报告占位、简历、JD、弱项占位、已问追问、当前主问题）和回答分析生成追问文案；rollback provider 不把候选人当前回答原文组成的对话记录注入追问 memory，并会确定性拒绝重复追问。
