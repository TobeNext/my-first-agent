...
- Report DB 由 `REPORT_DATABASE_URL` 配置，当前默认 SQLite 文件为 `./interview_reports.db`；`app.integrations.report_repository` 负责自动初始化 `interview_reports`、`interview_report_items`、`interview_report_reads`、`interview_user_memories` 和 `interview_user_memory_profiles`，最终 markdown 与结构化 report JSON 以 DB 为持久化真源，不应写入 checkpoint DB。用户长期面试记忆表属于报告 DB 的扩展基础设施；业务流程写入必须经由 domain tool 收敛，不能在 runner/node 中散落直接写 repository。
